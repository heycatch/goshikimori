...
func TestAnimeSchema(t *testing.T) {
  pass_normal := `graphql?query={animes(search: "initial d", page: 1, limit: 1, score: 8, order: id, kind: "tv", status: "!anons", season: "199x", duration: "F", rating: "!rx", mylist: "completed", censored: false){id name russian english japanese score airedOn{year month day date} }}`
  normal, _ := AnimeSchema(
    Values("id", "name", "russian", "english", "japanese", "score", "airedOn{year month day date}"),
    "initial d",
    1, 1, 8, "id", "tv", "!anons", "199x", "F", "!rx", "completed", false,
  )
...
func TestMangaSchema(t *testing.T) {
  pass_normal := `graphql?query={mangas(search: "initial d", page: 1, limit: 3, score: 8, order: ranked, kind: "manga", status: "released", mylist: "planned", censored: false){id name russian volumes chapters releasedOn{year month day date} url }}`
  normal, _ := MangaSchema(
    Values("id", "name", "russian", "volumes", "chapters", "releasedOn{year month day date}", "url"),
    "initial d",
    1, 3, 8, "ranked", "manga", "released", "", "planned", false,
  )
  if normal == pass_normal {
