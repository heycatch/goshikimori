...
func AnimeSchema(values string, name string, options ...interface{}) (string, error) {
  ...
  for i, option := range options {
    ...
    case 11:
      ...
      if ok_genre_v2 && len(genre_v2) > 0 {
        parameterOptions += ", genre: " + `"` + genre_v2[:len(genre_v2)-1] + `"`
      }
    default:
